...
//					console.log(webServerName + ' already running at port ' + serverCfg.port + '.');
					...
					daemon.start(function () {
						...
						fats.log.info(webServerName + ' now running at port ' + config.port);
			...
			var restartEvent = function () {
				...
				daemon = require('daemonize2').setup({
					...
					silent: true,
					pidfile: 'fatsDaemon.pid',
					argv: ['--root', root || serverCfg.root, '--port', port || serverCfg.port, '--watch', watch || serverCfg.watch]
				});

				daemon.stop(function (err) {
					daemon.start();

					fats.log.info(webServerName + ' now running at port ' + serverCfg.port);
			...
			switch (cmd) {
				...
				case 'status':
					var pid = daemon.status();
					if (pid){
						fats.log.info(webServerName + ' daemon running. PID: ' + pid);
					} else {
						fats.log.info(webServerName + ' is not running.');
					}
					process.exit(0);
					break;
